...
void init_sender(Sender * sender, int id)
{
    ...
    //添加
    sender->window->begin = 0;
    sender->window->frame_head = NULL;
    sender->window->number = 0;
    //添加
}
                          ...
                          LLnode ** outgoing_frames_head_ptr)
{
    ...
    head = sender->window->frame_head;
    LLnode * tmp = (LLnode *) malloc (sizeof(LLnode));
    Frame * input = (Frame *) malloc (sizeof(Frame));
    if (head != NULL)
    {
        head->mask = 2;
        tmp = head->next;
    ...
    while (input_frame_length > 0)
    {
        ...
        input = (Frame *)ll_input_frame_node->value;
        input_frame_length = ll_get_length(sender->input_cmdlist_head);
        LLnode ** ack_frame = find_serial(&sender->window->frame_head, input->serial);
        if(ack_frame != NULL)
        {
            ll_pop_node(ack_frame);
        }
    }
                       ...
                       LLnode ** outgoing_frames_head_ptr)
{
    ...
    int frame_window = ll_get_length(sender->window->frame_head);//添加
    


    while ((input_cmd_length > 0) && (frame_window < (MAX_WINDOWS)))
    {
        //Pop a node off and update the input_cmd_length
        ...
        else
        {
            ...
            //添加
            outgoing_frame->send = outgoing_cmd->src_id;
            outgoing_frame->receiver = outgoing_cmd->dst_id;
            outgoing_frame->ack = 1000;
            outgoing_frame->CRC8 = cal_crc_table(outgoing_cmd->message,strlen(outgoing_cmd->message));
            outgoing_frame->serial = (sender->window->number % MAX_SERIAL);
            ll_append_node(&sender->window->frame_head,
                                       (void *) outgoing_frame);
            sender->window->number++;
            frame_window = ll_get_length(sender->window->frame_head);//添加
            //添加
...
{    
    struct timespec   time_spec;//纳秒
    struct timeval    curr_timeval;//微秒
